...
static void tcp_event_data_recv(struct sock *sk, struct sk_buff *skb)
{
	...
	if (READ_ONCE(sock_net(sk)->ipv4.sysctl_tcp_aad)) {
		...
		if (!icsk->icsk_ack.ato) {
			 ...
			 * delayed ACK engine.
			 */
			pr_debug("TCP_AAD RECV now_us=%llu sk=%p seq=%u end_seq=%u len=%u | INIT\n", now_us, sk, TCP_SKB_CB(skb)->seq, TCP_SKB_CB(skb)->end_seq, skb->len);
			...
			if (iat_curr_us > jiffies_to_usecs(icsk->icsk_rto)) {
				//--- Stall detection ---
				pr_debug("TCP_AAD RECV now_us=%llu sk=%p seq=%u end_seq=%u | STALL iat=%llu rto=%u\n", now_us, sk, TCP_SKB_CB(skb)->seq, TCP_SKB_CB(skb)->end_seq, iat_curr_us, jiffies_to_usecs(icsk->icsk_rto));
				tcp_incr_quickack(sk, TCP_MAX_QUICKACKS);
				icsk->icsk_ack.iat_min_us  = U64_MAX;
			} else {
	...
	#endif
	{
		if (!icsk->icsk_ack.ato) {
			 ...
			 * delayed ACK engine.
			 */
			pr_debug("TCP_DACK RECV now=%u sk=%p seq=%u end_seq=%u len=%u | INIT\n", now, sk, TCP_SKB_CB(skb)->seq, TCP_SKB_CB(skb)->end_seq, skb->len);
			tcp_incr_quickack(sk, TCP_MAX_QUICKACKS);
			icsk->icsk_ack.ato = TCP_ATO_MIN;
		} else {
			...
			if (m <= TCP_ATO_MIN / 2) {
				...
				pr_debug("TCP_DACK RECV now=%u sk=%p seq=%u end_seq=%u | FAST m=%d ato=%u\n", now, sk, TCP_SKB_CB(skb)->seq, TCP_SKB_CB(skb)->end_seq, m, icsk->icsk_ack.ato);
			} else if (m < icsk->icsk_ack.ato) {
				icsk->icsk_ack.ato = (icsk->icsk_ack.ato >> 1) + m;
				if (icsk->icsk_ack.ato > icsk->icsk_rto)
					icsk->icsk_ack.ato = icsk->icsk_rto;
				pr_debug("TCP_DACK RECV now=%u sk=%p seq=%u end_seq=%u | MID m=%d ato=%u\n", now, sk, TCP_SKB_CB(skb)->seq, TCP_SKB_CB(skb)->end_seq, m, icsk->icsk_ack.ato);
			} else if (m > icsk->icsk_rto) {
				/* Too long gap. Apparently sender failed to
				 * restart window, so that we send ACKs quickly.
				 */
				tcp_incr_quickack(sk, TCP_MAX_QUICKACKS);
				pr_debug("TCP_DACK RECV now=%u sk=%p seq=%u end_seq=%u | STALL m=%d rto=%u\n", now, sk, TCP_SKB_CB(skb)->seq, TCP_SKB_CB(skb)->end_seq, m, icsk->icsk_rto);
			}
		}
	}
...
send_now:
	...
	#ifdef CONFIG_TCP_AAD
	if (READ_ONCE(net->ipv4.sysctl_tcp_aad)) {
		pr_debug("TCP_AAD ACK_CHK sk=%p rcv_nxt=%u COMPRESSED compressed_ack=%u delay_us=%llu\n",
			 sk, tp->rcv_nxt, tp->compressed_ack,
			 (u64)delay / NSEC_PER_USEC);
	} else
	#endif
	{
		pr_debug("TCP_DACK ACK_CHK sk=%p rcv_nxt=%u COMPRESSED compressed_ack=%u delay_us=%llu\n",
			 sk, tp->rcv_nxt, tp->compressed_ack,
			 (u64)delay / NSEC_PER_USEC);
	}
	...
	if (!inet_csk_ack_scheduled(sk)) {
		...
		if (READ_ONCE(sock_net(sk)->ipv4.sysctl_tcp_aad)) {
			...
			pr_debug("TCP_AAD ACK_CHK sk=%p rcv_nxt=%u SKIP pending=%x quick=%u\n",
				 sk, tcp_sk(sk)->rcv_nxt, icsk->icsk_ack.pending, icsk->icsk_ack.quick);
		} else
		#endif
		{
			struct inet_connection_sock *icsk = inet_csk(sk);
			pr_debug("TCP_DACK ACK_CHK sk=%p rcv_nxt=%u SKIP pending=%x quick=%u\n",
				 sk, tcp_sk(sk)->rcv_nxt, icsk->icsk_ack.pending, icsk->icsk_ack.quick);
		}
		return;
	}
