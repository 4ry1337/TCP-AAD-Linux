...
static __cold void tcp_gro_dev_warn(const struct sock *sk,
				    const struct sk_buff *skb, unsigned int len)
{
...
static void tcp_measure_rcv_mss(struct sock *sk, const struct sk_buff *skb)
{
	...
	if (len >= icsk->icsk_ack.rcv_mss) {
		...
		DO_ONCE_LITE_IF(len > icsk->icsk_ack.rcv_mss +
				      ...
				      u32 delivered_bytes, int flag)
{
	...
	if (tp->saw_accecn_opt < TCP_ACCECN_OPT_COUNTER_SEEN) {
		tp->saw_accecn_opt =
			tcp_accecn_option_init(skb, tp->rx_opt.accecn);
				...
				int flag)
{
	...
	if (opt_deltas_valid) {
		...
		if ((delivered_pkts >= (TCP_ACCECN_CEP_ACE_MASK + 1) * 2) &&
		    (tcp_is_sack(tp) || ((1 << inet_csk(sk)->icsk_ca_state) &
			...
			est_d_cep = DIV_ROUND_UP_ULL(
				(u64)d_ceb * delivered_pkts, delivered_bytes);
		...
		if (d_ceb > delta * tp->mss_cache)
			return safe_delta;
		if (d_ceb < safe_delta * tp->mss_cache >>
			      ...
			      int *flag)
{
	...
	delta = __tcp_accecn_process(sk, skb, delivered_pkts, delivered_bytes,
				     *flag);
	if (delta > 0) {
...
static void tcp_event_data_recv(struct sock *sk, struct sk_buff *skb)
{
...
#endif
	{
		if (!icsk->icsk_ack.ato) {
			 ...
			 */
			pr_debug(
				"TCP_DACK RECV now=%u sk=%p seq=%u end_seq=%u len=%u | INIT\n",
				now, sk, TCP_SKB_CB(skb)->seq,
				TCP_SKB_CB(skb)->end_seq, skb->len);
			tcp_incr_quickack(sk, TCP_MAX_QUICKACKS);
			icsk->icsk_ack.ato = TCP_ATO_MIN;
		} else {
			...
			if (m <= TCP_ATO_MIN / 2) {
				...
				icsk->icsk_ack.ato = (icsk->icsk_ack.ato >> 1) +
						     TCP_ATO_MIN / 2;
				pr_debug(
					"TCP_DACK RECV now=%u sk=%p seq=%u end_seq=%u mss=%u | FAST m=%d ato=%u\n",
					now, sk, TCP_SKB_CB(skb)->seq,
					TCP_SKB_CB(skb)->end_seq,
					icsk->icsk_ack.rcv_mss, m,
					icsk->icsk_ack.ato);
			} else if (m < icsk->icsk_ack.ato) {
				icsk->icsk_ack.ato =
					(icsk->icsk_ack.ato >> 1) + m;
				if (icsk->icsk_ack.ato > icsk->icsk_rto)
					icsk->icsk_ack.ato = icsk->icsk_rto;
				pr_debug(
					"TCP_DACK RECV now=%u sk=%p seq=%u end_seq=%u mss=%u | MID m=%d ato=%u\n",
					now, sk, TCP_SKB_CB(skb)->seq,
					TCP_SKB_CB(skb)->end_seq,
					icsk->icsk_ack.rcv_mss, m,
					icsk->icsk_ack.ato);
				...
				tcp_incr_quickack(sk, TCP_MAX_QUICKACKS);
				pr_debug(
					"TCP_DACK RECV now=%u sk=%p seq=%u end_seq=%u mss=%u | STALL m=%d rto=%u\n",
					now, sk, TCP_SKB_CB(skb)->seq,
					TCP_SKB_CB(skb)->end_seq,
					icsk->icsk_ack.rcv_mss, m,
					icsk->icsk_rto);
...
static u8 tcp_sacktag_one(struct sock *sk, struct tcp_sacktag_state *state,
			  ...
			  int pcount, u32 plen, u64 xmit_time)
{
	struct tcp_sock *tp = tcp_sk(sk);
			    ...
			    bool dup_sack)
{
	...
	tcp_sacktag_one(sk, state, TCP_SKB_CB(skb)->sacked, start_seq, end_seq,
			dup_sack, pcount, skb->len, tcp_skb_timestamp_us(skb));
	...
	skb_rbtree_walk_from(skb)
	{
		...
		if (in_sack) {
			TCP_SKB_CB(skb)->sacked = tcp_sacktag_one(
				...
				dup_sack, tcp_skb_pcount(skb), skb->len,
			       ...
			       struct tcp_sacktag_state *sack, bool ece_ack)
{
	...
	if (flag & FLAG_ACKED) {
		...
		sack->delivered_bytes =
			(skb ? TCP_SKB_CB(skb)->seq : tp->snd_una) -
			prior_snd_una;
...
static int tcp_ack(struct sock *sk, const struct sk_buff *skb, int flag)
{
	...
	if (tcp_ecn_mode_accecn(tp))
		ecn_count =
			tcp_accecn_process(sk, skb, tp->delivered - delivered,
 ...
 */
int tcp_do_parse_auth_options(const struct tcphdr *th, const u8 **md5_hash,
			      const u8 **ao_hash)
{
						     ...
						     const struct sk_buff *skb)
{
	...
	if (tcp_may_update_window(tp, ack, seq,
...
static enum skb_drop_reason tcp_sequence(const struct sock *sk, u32 seq,
					 u32 end_seq)
{
	const struct tcp_sock *tp = tcp_sk(sk);
...
noinline_for_tracing static void tcp_drop_reason(struct sock *sk,
						 struct sk_buff *skb,
...
static int tcp_try_rmem_schedule(struct sock *sk, const struct sk_buff *skb,
				 unsigned int size)
{
	if (!tcp_can_ingest(sk, skb) || !sk_rmem_schedule(sk, skb, size)) {
...
static void __tcp_ack_snd_check(struct sock *sk, int ofo_possible)
{
	...
	{
		pr_debug(
			"TCP_DACK ACK_CHK sk=%p rcv_nxt=%u ofo=%d bytes=%u mss=%u quick=%u pingpong=%d pending=%x\n",
			sk, tp->rcv_nxt, ofo_possible,
			tp->rcv_nxt - tp->rcv_wup, icsk->icsk_ack.rcv_mss,
			icsk->icsk_ack.quick, inet_csk_in_pingpong_mode(sk),
			icsk->icsk_ack.pending);
	...
	if (two_segs || quick || ack_now) {
		...
		if (sock_owned_by_user_nocheck(sk) &&
			...
			{
				pr_debug(
					"TCP_DACK ACK_CHK sk=%p rcv_nxt=%u DEFERRED quick=%u\n",
					sk, tp->rcv_nxt, icsk->icsk_ack.quick);
			}
...
send_now:
		...
		if (READ_ONCE(net->ipv4.sysctl_tcp_aad)) {
			pr_debug(
				...
				dup_ack ? "DUP_ACK " : "",
				icsk->icsk_ack.rcv_mss, icsk->icsk_ack.quick,
	...
	if (!ofo_possible || RB_EMPTY_ROOT(&tp->out_of_order_queue)) {
		...
		{
			pr_debug(
				"TCP_DACK ACK_CHK sk=%p rcv_nxt=%u DELAYED quick=%u pingpong=%d ato=%u\n",
				sk, tp->rcv_nxt, icsk->icsk_ack.quick,
				inet_csk_in_pingpong_mode(sk),
				icsk->icsk_ack.ato);
		}
	...
	if (!tcp_is_sack(tp) ||
	    tp->compressed_ack >=
		    READ_ONCE(net->ipv4.sysctl_tcp_comp_sack_nr)) {
		comp_limit = true;
		goto send_now;
	}
	...
	{
		pr_debug(
			"TCP_DACK ACK_CHK sk=%p rcv_nxt=%u COMPRESSED compressed_ack=%u delay_us=%llu\n",
			sk, tp->rcv_nxt, tp->compressed_ack,
			(u64)delay / NSEC_PER_USEC);
	}
...
/* We sent a data segment already. */
...
#endif
		{
			struct inet_connection_sock *icsk = inet_csk(sk);
			pr_debug(
				"TCP_DACK ACK_CHK sk=%p rcv_nxt=%u SKIP pending=%x quick=%u\n",
				sk, tcp_sk(sk)->rcv_nxt, icsk->icsk_ack.pending,
				icsk->icsk_ack.quick);
		}
		return;
	...
	if (th->syn) {
		if (tcp_ecn_mode_accecn(tp)) {
			...
			if (tp->rx_opt.accecn &&
				...
				u8 saw_opt = tcp_accecn_option_init(
					skb, tp->rx_opt.accecn);
...
consume:
		...
		if (sk->sk_write_pending ||
			...
			inet_csk_schedule_ack(sk);
			tcp_enter_quickack_mode(sk, TCP_MAX_QUICKACKS);
			tcp_reset_xmit_timer(sk, ICSK_TIME_DACK, TCP_DELACK_MAX,
					     false);
			goto consume;
		}
...
enum skb_drop_reason tcp_rcv_state_process(struct sock *sk, struct sk_buff *skb)
{
	...
	case TCP_SYN_RECV:
		...
		if (req) {
			...
			tp->retrans_stamp = 0;
			tcp_init_transfer(
				sk, BPF_SOCK_OPS_PASSIVE_ESTABLISHED_CB, skb);
			WRITE_ONCE(tp->copied_seq, tp->rcv_nxt);
		}
				   ...
				   const struct dst_entry *dst)
{
	...
	if (((!ect || th->res1 || th->ae) && ecn_ok) ||
	    tcp_ca_needs_ecn(listen_sk) || (ecn_ok_dst & DST_FEATURE_ECN_CA) ||
	...
	if (syncookies != 2 && !READ_ONCE(queue->synflood_warned)) {
		...
		if (IS_ENABLED(CONFIG_IPV6) && sk->sk_family == AF_INET6) {
			net_info_ratelimited(
				...
				proto, inet6_rcv_saddr(sk), sk->sk_num, msg);
		} else {
			net_info_ratelimited(
				"%s: Possible SYN flooding on port %pI4:%u. %s.\n",
		     ...
		     struct sk_buff *skb)
{
	...
	if (isn) {
		...
		if (syncookies == 2 || inet_csk_reqsk_queue_is_full(sk)) {
			want_cookie =
				tcp_syn_flood_action(sk, rsk_ops->slab_name);
			if (!want_cookie)
				goto drop;
		}
	}
